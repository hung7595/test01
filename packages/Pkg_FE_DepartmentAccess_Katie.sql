CREATE OR REPLACE PACKAGE Pkg_FE_DepartmentAccess_Katie
AS
...
END Pkg_FE_DepartmentAccess_Katie;
/

CREATE OR REPLACE PACKAGE BODY Pkg_FE_DepartmentAccess_Katie
IS
  PROCEDURE IsLeaf (
    iPdeptId IN INT,
    iPsiteId IN INT,
        ...
        (
          (
            CASE cLsite_prefix
              ...
              WHEN 'us_ws' THEN us_ws_enabled
              WHEN 'jp' THEN jp_enabled
              WHEN 'hk' THEN hk_enabled
        ...
        WHERE
          ...
          AND EXISTS
            (
              ...
              WHERE
               p.productid=pr.sku
                AND p.regionId=iPsiteId
                AND p.enable = 'Y'
                AND pd.sku = p.productid
          ...
          AND exists
            (
              ...
              WHERE 1=1
                ...
                AND pd.dept_id = d.dept_id
                AND pr.supplierid = iPsupplierId
            )
        ORDER BY d.us_disp_seq, d.dept_id, dl.lang_id;
