...
PACKAGE BODY Pkg_FE_GenerateImageButton AS
  ...
  BEGIN
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE tmp_toys_character_pool';
  ...
  BEGIN
    ...
    SELECT seq_artist_image_button_log.nextval, iPdept_id, a.artist_id, sysdate, 1 FROM (SELECT a.artist_id FROM ya_artist_image a, ya_artist_dept d WHERE a.artist_id = d.artist_id AND a.type=5 AND d.dept_id = iPdept_id and rownum<=15) a
       WHERE NOT EXISTS(  
