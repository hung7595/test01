...
END Pkg_fe_PaypalAccess3;
/

create or replace
PACKAGE BODY PKG_FE_PAYPALACCESS3
IS
PROCEDURE InsertDetailLog (
    iPpaypalDataType IN INT,
    cPpaypalUserId IN VARCHAR2,
  ...
  END GetTxnIdIdbyPaymentTxnId;
	...
	BEGIN
		OPEN curPresult FOR
            ...
            from ya_paypal_status ps inner join ya_paypal_order_mapping om on ps.payment_uid=om.payment_uid
            where
            (order_num is null and backend_status is null and payment_status=1 and
			ps.order_status=-2 and ps.created_datetime <= to_date(sysdate-1) and status in(0,-1))
            --not yet process date > sysdate +1
            or (order_status=-1 and status in (0,-1) and
			((order_num>0 and backend_status>0 and backend_status<3) or order_num is null)) --no response po
            or (notify_response in (1,2) and order_status in (0,1,-1) and
			((order_num>0 and backend_status>0 and backend_status<3) or (order_num is null and status in (0,-1))))
            order by ps.payment_uid;
	...
	BEGIN
		OPEN curPresult FOR
			...
			,pial.transaction_id, pial.payer_country, pial.payer_state, pial.payer_zip, pial.payer_city, pial.payer_street
			from
			ya_paypal_order_mapping pom
			inner join ya_paypal_detail_log pil on (
            paypal_data_type=2 and
     		(pom.paypal_status = 4 -- pending
            or pom.paypal_status = 1 and pom.status = 1 and pil.payment_status =1  --pending
     		or pom.paypal_status = 1 and pom.status = 6 and pil.payment_status =1  --amount increase
    		or pom.paypal_status = 1 and pom.status = 7 and pil.payment_status =1  -- amount decrease
     		or pom.paypal_status = 1 and pom.status = 10 and pil.payment_status =1 ) -- payer unverified
			...
			left join (
                ...
                paypal_data_type=2 and
    		    (pom.paypal_status = 4
				or pom.paypal_status = 1 and pom.status = 1 and pil.payment_status =1
     		    or pom.paypal_status = 1 and pom.status = 6 and pil.payment_status =1
     		    or pom.paypal_status = 1 and pom.status = 7 and pil.payment_status =1
     		    or pom.paypal_status = 1 and pom.status = 10 and pil.payment_status =1 )
				and pil.parent_transaction_id = pom.txn_id )
	...
	BEGIN
		OPEN curPresult FOR
				...
				dl.transaction_id,  om.paypal_status, om.status ,dl.payer_user_email as payer_email,
				dl.payment_amount as amount,  dl.pending_reason, dl.payer_status, dl.payment_datetime,om.created_datetime,
				ps.notify_response,ps.order_status
			from ya_paypal_detail_log dl
			left join ya_paypal_order_mapping om on dl.transaction_id=om.txn_id and dl.paypal_data_type=2
			inner join ya_paypal_status ps on om.payment_uid=ps.payment_uid and ps.paypal_data_type=2
            --inner join ya_shopper s on om.shopper_id=s.shopper_id
            --left join order_info oi on oi.cust_id=s.shopper_id
			where ((paypal_status=1 and ps.order_status=-2 and ps.created_datetime <= to_date(sysdate-1))
            or (paypal_status=1 and ps.order_status not in (-2)))
            and order_num is null and status<>4
            and dl.transaction_id not in (select referencenumber from creditSource where referencenumber in
			(select distinct transaction_id from ya_paypal_detail_log))
            group by
				om.shopper_id,dl.payer_user_email,ps.payment_uid,ps.notify_response,ps.payment_status,ps.order_status,
				dl.payer_firstname, dl.payer_lastname,dl.transaction_id,  om.paypal_status, om.status ,dl.payer_user_email,
				dl.payment_amount ,  dl.pending_reason, dl.payer_status, dl.payment_datetime,om.created_datetime,
				ps.notify_response,ps.order_status
			order by created_datetime desc;
